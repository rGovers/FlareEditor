...
void MonoProjectGenerator::Serialize(const std::string_view& a_name, const std::filesystem::path& a_path, const std::filesystem::path& a_outPath) const
{
	...
	if (m_dependencyCount > 0)
	{
		...
		for (uint32_t i = 0; i < m_dependencyCount; ++i)
		{
			...
			if (m_dependencies[i] == "FlareEngine")
			{
				const std::filesystem::path cwd = std::filesystem::current_path();
				const std::filesystem::path p = cwd / "FlareCS.dll";
			...
			else if (m_dependencies[i] == "FlareEditor")
			{
				const std::filesystem::path cwd = std::filesystem::current_path();
				const std::filesystem::path p = cwd / "FlareEditorCS.dll";
